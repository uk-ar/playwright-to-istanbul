...
class OutputFiles {
  ...
  rewritePath(path) {
    ...
    let dir = pathLib.dirname(parsedPath);
    let ext = pathLib.extname(parsedPath);
    if (ext != ".js")//instanbul works with only ".js" ext.
    ...
    if(fs.existsSync(parsedPath) && ext != ".js" && ext != ".css"){      
      ...
      return pathLib.format({//foo.html.js
        dir: dir, base: pathLib.basename(parsedPath, ".js") + "-" + iterator[parsedPath] + ".js"
      });
    } else {
      return parsedPath;
  ...
  async _parseAndIsolate() {
    for (let i = 0; i < this.coverageInfo.length; i++) {
      ...
      if (this.coverageInfo[i].source)
        fs.writeFileSync(path, this.coverageInfo[i].source)
      else
